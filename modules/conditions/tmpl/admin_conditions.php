...
<div id="jsvehiclemanageradmin-wrapper">
    ...
    <div id="jsvehiclemanageradmin-data">
        ...
        <div class="jsvm_page-actions">
            <a class="jsvm_js-bulk-link button jsvm_multioperation" message="<?php echo JSVEHICLEMANAGERmessages::getMSelectionEMessage(); ?>" data-for="publish" data-for-wpnonce="<?php echo wp_create_nonce("publish-condition"); ?>" href="#"><img src="<?php echo jsvehiclemanager::$_pluginpath; ?>includes/images/publish-icon.png" /><?php echo __('Publish', 'js-vehicle-manager') ?></a>
            <a class="jsvm_js-bulk-link button jsvm_multioperation" message="<?php echo JSVEHICLEMANAGERmessages::getMSelectionEMessage(); ?>" data-for="unpublish" data-for-wpnonce="<?php echo wp_create_nonce("unpublish-condition"); ?>" href="#"><img src="<?php echo jsvehiclemanager::$_pluginpath; ?>includes/images/unbuplish.png" /><?php echo __('Unpublished', 'js-vehicle-manager') ?></a>
        ...
        if (!empty(jsvehiclemanager::$_data[0])) {
            ...
            <form id="jsvehiclemanager-list-form" method="post" action="<?php echo admin_url("admin.php?page=jsvm_conditions"); ?>">
                    ...
                    <tbody>
                        ...
                        for ($i = 0, $n = count(jsvehiclemanager::$_data[0]); $i < $n; $i++) {
                            ...
                            <tr valign="top">
                                ...
                                <td>
                                    <?php if ($row->status == 1) { ?> 
                                        <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=jsvm_conditions&task=unpublish&action=jsvmtask&jsvehiclemanager-cb[]='.$row->id.$pageid),'unpublish-condition'); ?>">
                                    ...
                                    <?php } else { ?>
                                        <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=jsvm_conditions&task=publish&action=jsvmtask&jsvehiclemanager-cb[]='.$row->id.$pageid),'publish-condition'); ?>">
                                ...
                                <td>
                                    <?php if ($i != 0 || $pagenum > 1) { ?>
                                        <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=jsvm_common&task=defaultorderingup&action=jsvmtask&for=conditions&id='.$row->id.$pageid.'&jsvmlt=conditions'),'field-up'); ?>">
                                    ...
                                    <?php if ($i < $n - 1 || $islastordershow) { ?>
                                        <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=jsvm_common&task=defaultorderingdown&action=jsvmtask&for=conditions&id='.$row->id.$pageid.'&jsvmlt=conditions'),'field-down'); ?>">
                ...
                <?php echo JSVEHICLEMANAGERformfield::hidden('_wpnonce', wp_create_nonce('delete-condition')); ?>
            </form>
            <?php
